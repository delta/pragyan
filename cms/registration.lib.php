...
<form class="registrationform"  method="POST" name="user_reg_usrFrm" onsubmit="return checkRegistrationForm(this)" action="./+login&subaction=register">
	<fieldset>
...
function register() {
	...
	elseif ((isset ($_GET['reSendKey'])) && (!isset ($_POST['resend_key_email']))) {

		$reSendForm =<<<FORM
<form  class="registrationform" method="POST" name="user_resend_key" onsubmit="return checkForm(this)" action="./+login&subaction=register&reSendKey">
   <fieldset>
...
FORM;
	...
	elseif (isset ($_POST['resend_key_email'])) {
		...
		if (!mysql_num_rows($result))
			...
			if ($temp['user_activated'] == 1)
				...
				"Thank you for signing up with Pragyan08.\n" .
				"Kindly use the following link to activate your account.\n" .
				"http://pragyan.org/08/home/+login&subaction=register&verify=$email&key=$key\n" .
				"(Copy and paste the given link in your browser window if required )\n" .
				"See you at Pragyan08\n" .
				"From Feb 28th to March 2nd 2008.\n" .
				"Lets Celebrate technology\n" .
				"\n\n\n\n\n\n\nThis mail was sent because you or someone registered with pragyan.org with this mail id.\n" .
				"If you did not register with pragyan.org then kindly send a mail to webadmin@pragyan.org\n" .
				"with \"Mail removal\" as the subject and a copy of this mail\n";

				$to = "$email";
				$subject = "Pragyan 2008 registration";
				$body = "";
	...
	elseif (isset ($_GET['key'])) {
		...
		$temp = mysql_fetch_assoc($result);
		if ($temp['user_activated'] == 1)
			displayinfo("E-mail $_GET[verify] has already been verified");
		else {
			if ($_GET['key'] == getVerificationKey($_GET['verify'], $temp['user_password'], $temp['user_regdate'])) {
				...
				if (mysql_affected_rows() > 0)
					displayinfo("Your e-mail $_GET[verify] has been verified. Now you can fill your profile information by clicking <a href=\"./+profile\">here</a> or by clicking on the preferences link in the action bar any time you are logged in.");
				else
					displayerror("Verification error for $_GET[verify]. Please contact webadmin@pragyan.org");
			} else
				displayerror("Verification error for $_GET[verify]. Please contact webadmin@pragyan.org");
		}
	}
	///Registration form submission
		...
		if (mysql_num_rows($result)) {
			...
			$query = "INSERT INTO `" . MYSQL_DATABASE_PREFIX . "users` " .
					"(`user_name`, `user_email`, `user_fullname`, `user_password`) " .
					"VALUES ('{$_POST['user_name']}', '{$_POST['user_email']}', '{$_POST['user_fullname']}', '$passwd')";
			mysql_query($query);
			...
			$message = "Dear {$temp['user_fullname']},\n" .
			"Thank you for signing up with Pragyan'08.\n" .
			"Kindly use the following link to activate your account.\n" .
			"http://pragyan.org/08/home/+login&subaction=register&verify=$temp[user_email]&key=$key\n" .
			"(Copy and paste the given link in your browser window if required )\n" .
			...
			"\n\n\n\n\n\n\nThis mail was sent because you or someone registered with pragyan.org with this mail id.\n" .
				...
				"with \"Mail removal\" as the subject and a copy of this mail\n";
			$to = "{$temp['user_email']}";
			$subject = "Pragyan'08 registration";
			$from = "From:no-reply@pragyan.org";
			if (mail($to, $subject, $message, $from)) {
				displayinfo("Sign Up for Pragyan 08 successful. Kindly <b>check your e-mail </b>for the account activation link to activate your account.");
			} else {
				displayerror("Sign Up failed. Kindly contact webadmin@pragyan.org");
			}
		}
	}
}
