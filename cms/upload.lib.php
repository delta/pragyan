...
function upload($moduleComponentId, $moduleName, $userId, $uploadFormName, $maxFileSizeInBytes=false, $uploadableFileTypesArray = false) {
	...
	if (!file_exists($uploadDir)) {
		displaywarning("The file $uploadDir does not exist. Trying to creating it.");
	...
	if (!file_exists($uploadDir . '/' . $moduleName)) {
		displaywarning("The file ".$uploadDir.'/'.$moduleName." does not exist. Trying to create it");
...
function getUploadedFilePreviewDeleteForm($moduleComponentId, $moduleName, $deleteFormAction = './+edit') {
	...
	foreach($uploadedFiles as $file) {
		...
		<tr>
			...
			<td><input type='submit' value='Delete'  onclick="return checkDelete(this, '$fileDelete');"></td>
		</tr>
UPLOADEDFILESSTRING;
	}
