...
$ent = $sph_entities;
	...
	function makeboollist($a) {
		global $sph_entities, $sph_stem_words;

		while ($char = each($sph_entities)) {
...
//		$a = remove_accents($a);
	...
	function ignoreword($word) {
		global $sph_common;
		global $sph_min_word_length;
		global $sph_index_numbers;
		if ($sph_index_numbers == 1) {
			$pattern = "[a-z0-9]+";
		} else {
			$pattern = "[a-z]+";
		}
		if (strlen($word) < $sph_min_word_length || (!eregi($pattern, remove_accents($word))) || ($sph_common[$word] == 1)) {
			return 1;
		} else {
			return 0;
		}
	...
	function search($searchstr, $category, $start, $per_page, $type, $domain) {
		global $sph_length_of_link_desc, $sph_mysql_table_prefix, $sph_show_meta_description, $sph_merge_site_results, $sph_stem_words, $sph_did_you_mean_enabled;

		$possible_to_find = 1;
		$result = mysql_query("select domain_id from ".$sph_mysql_table_prefix."domains where domain = '$domain'");
		...
		while ($not_words < count($wordarray)) {
            ...
            $query1 = "SELECT link_id from ".$sph_mysql_table_prefix."link_keyword$wordmd5, ".$sph_mysql_table_prefix."keywords where ".$sph_mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$sph_mysql_table_prefix."keywords.keyword_id and keyword='$searchword'";
		...
		while ($phrase_words < count($wordarray)) {
			...
			$query1 = "SELECT link_id from ".$sph_mysql_table_prefix."links where fulltxt like '% $searchword%'";
			echo mysql_error();
			$result = mysql_query($query1);
		...
		if (($category> 0) && $possible_to_find==1) {
			...
			$query1 = "select link_id from ".$sph_mysql_table_prefix."links, ".$sph_mysql_table_prefix."sites, ".$sph_mysql_table_prefix."categories, ".$sph_mysql_table_prefix."site_category where ".$sph_mysql_table_prefix."links.site_id = ".$sph_mysql_table_prefix."sites.site_id and ".$sph_mysql_table_prefix."sites.site_id = ".$sph_mysql_table_prefix."site_category.site_id and ".$sph_mysql_table_prefix."site_category.category_id in ($catlist)";
			$result = mysql_query($query1);
			echo mysql_error();
			$num_rows = mysql_num_rows($result);
			if ($num_rows == 0) {
		...
		while (($words < count($wordarray)) && $possible_to_find == 1) {
			...
			$wordmd5 = substr(md5($searchword), 0, 1);
			$query1 = "SELECT distinct link_id, weight, domain from ".$sph_mysql_table_prefix."link_keyword$wordmd5, ".$sph_mysql_table_prefix."keywords where ".$sph_mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$sph_mysql_table_prefix."keywords.keyword_id and keyword='$searchword' $domain_qry order by weight desc";
			echo mysql_error();
			$result = mysql_query($query1);
			$num_rows = mysql_num_rows($result);
			if ($num_rows == 0) {
		...
		if ((count($result_array_full) == 0 || $possible_to_find == 0) && $sph_did_you_mean_enabled == 1) {
			reset ($searchstr['+']);
			foreach ($searchstr['+'] as $word) {
				$word = addslashes($word);
				$result = mysql_query("select keyword from ".$sph_mysql_table_prefix."keywords where soundex(keyword) = soundex('$word')");
				$max_distance = 100;
				$near_word ="";
				while ($row=mysql_fetch_row($result)) {
		...
		if ($sph_merge_site_results == 1 && $domain_qry == "") {
		...
		if ($sph_length_of_link_desc == 0) {
			$fulltxt = "fulltxt";
		} else {
			$fulltxt = "substring(fulltxt, 1, $sph_length_of_link_desc)";
		}

		$query1 = "SELECT distinct link_id, url, title, description,  $fulltxt, size FROM ".$sph_mysql_table_prefix."links WHERE link_id in ($inlist)";
		...
		while ($row = mysql_fetch_row($result)) {
			$res[$i]['title'] = $row[2];
			$res[$i]['url'] = $row[1];
			if ($row[3] != null && $sph_show_meta_description == 1)
				$res[$i]['fulltxt'] = $row[3];
			else 
				$res[$i]['fulltxt'] = $row[4];
			$res[$i]['size'] = $row[5];
			$res[$i]['weight'] = $result_array[$row[0]];
			$dom_result = mysql_query("select domain from ".$sph_mysql_table_prefix."domains where domain_id='".$domains[$row[0]]."'");
		...
		if ($sph_merge_site_results  && $domain_qry == "") {
...
function get_search_results($query, $start, $category, $searchtype, $results, $domain) {
	global $sph_messages, $sph_results_per_page,
		$sph_links_to_next,
		$sph_show_query_scores,
		$sph_mysql_table_prefix,
		$sph_desc_length;
	global $sph_entities;

	if ($results != "")	$sph_results_per_page = $results;
	...
	$result = search($words, $category, $start, $sph_results_per_page, $searchtype, $domain);
	...
	$from = ($start-1) * $sph_results_per_page+1;
	$to = min(($start)*$sph_results_per_page, $rows);
	...
	if ($rows>0) {
		...
		$i = 0;
		while ($i < $num_of_results && $i < $sph_results_per_page) {
			...
			if ($txtlen > $sph_desc_length) {
				...
				while(list($id, $place) = each($places)) {
					while ($places[$id + $x] - $place < $sph_desc_length && $x+$id < count($places) && $place < strlen($fulltxt) -$sph_desc_length) {
						$x++;
						$begin = $id;
						$end = $id + $x;
					}
				...
				$fulltxt = substr($fulltxt, $begin_pos, $sph_desc_length);
	...
	$pages = ceil($rows / $sph_results_per_page);
	...
	if ($from <= $to) {
		...
		$firstpage = $start - $sph_links_to_next;
		if ($firstpage < 1) $firstpage = 1;
		$lastpage = $start + $sph_links_to_next;
