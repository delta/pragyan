...
********************************************/
	...
	$includes = array('./include', 'include', '../include', "$sourceFolder/$moduleFolder/search/include");
	...
	function get_cats($parent) {
		...
		$query = "SELECT * FROM ".$sph_mysql_table_prefix."categories WHERE parent_num=$parent";
		echo mysql_error();
		$result = mysql_query($query);
		$arr[] = $parent;
	...
	$GLOBALS['sph_entities'] = $entities;
	$GLOBALS['sph_apache_indexes'] = $apache_indexes;
...
function replace_ampersand($str) {
	...
	*/
	...
	{
		global $sph_regex_vowel, $sph_regex_consonant;
		// Part a
		if (substr($word, -1) == 's') {
	...
	*/
	function cvc($str)
	{
		global $sph_regex_consonant, $sph_regex_vowel;
		$c = $sph_regex_consonant;
		$v = $sph_regex_vowel;
