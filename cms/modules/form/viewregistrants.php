...
function generateFormDataTable($moduleComponentId, $sortField, $sortOrder, $action = 'viewregistrants') {
	global $sourceFolder, $templateFolder, $urlRequestRoot;
	...
	$javascriptBody = '<script language="javascript">
	...
	if($action == 'editregistrants') {
		$editRegistrantsView .= <<<EDITREGISTRANTSVIEW
			<form name="addusertoformform" method="POST" action="./+editregistrants">
				<script type="text/javascript" language="javascript" src="$urlRequestRoot/$sourceFolder/$templateFolder/common/scripts/ajaxsuggestionbox.js">
				</script>
...
EDITREGISTRANTSVIEW;
	...
	$editRegistrantsView .= $javascriptBody.'<table border="1">' . $tableCaptions . $tableBody . '</table><br />';
	if($action == 'editregistrants') {
