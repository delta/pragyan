 ...
 */
	...
	function generateRegistrationForm($moduleCompId,$userId,$action="", $disableCaptcha = false) {
		...
		$jsPath2 = "$urlRequestRoot/$sourceFolder/$moduleFolder/form/validation.js";//validation.js
		$jsPath = "$urlRequestRoot/$sourceFolder/templates/common/scripts/formValidator.js";//validation.js
		$calpath = "$urlRequestRoot/$sourceFolder/$moduleFolder";

		$body = '<script language="javascript" type="text/javascript" src="'.$jsPath2.'"></script><script language="javascript" type="text/javascript" src="'.$jsPath.'"></script><script>
window.addEvent("domready", function() {
	var exValidatorA = new fValidator("registrationform");
		...
		$body .= '<br /><br /><div class="registrationform"><form id="registrationform" class="fValidator-form" action="'.$action.'" onsubmit="return validate_form(this)" method="post"';
		...
		$body .= '</table></fieldset>' .
							'<br /><input type="submit" name="submitreg_form_'.$moduleCompId.'" value="Register" />' .
...
SCRIPT;
	...
	function getCaptchaHtml() {
			...
			$captcha = new captcha($sourceFolder, $moduleFolder, $uploadFolder, $urlRequestRoot,6);
			...
			$body = '<tr><td>Enter the text as shown in the image :</td><td>' .
					...
					'<input type="text" class="fValidate[\'required\']" name="txtCaptcha" /><td></tr>';
			return $body;
	}
...
function getFormElementInputField($moduleComponentId, $elementId, $value="", &$javascriptCheckFunctions) {
	...
	return $htmlOutput . "</td>\n";
		...
		function getFormElementTextarea($elementName,$value,$isRequired,$elementHelpName,$elementTooltip,$elementSize,$elementTypeOptions,$elementMoreThan,$elementLessThan,$elementCheckInt,&$jsOutput,&$htmlOutput){
			$validCheck='';
			if($isRequired)
				$validCheck=" class=\"fValidate['required']\"";
		...
		function getFormElementText($elementName,$value,$isRequired,$elementHelpName,$elementTooltip,$elementSize,$elementTypeOptions,$elementMoreThan,$elementLessThan,$elementCheckInt,&$jsOutput,&$htmlOutput)
		 {
			...
			{
				if($validCheck!="")
				$validCheck=" class=\"fValidate['required','num']\"";
				else
				$validCheck=" class=\"fValidate['num']\"";

				$jsOutput[] = "checkInt('$elementName', '$elementHelpName')";
				if(!is_null($elementMoreThan) && $elementMoreThan!="")
					$jsOutput[] = "checkLBInt('$elementName', {$elementMoreThan}, '$elementHelpName')";
				if(!is_null($elementLessThan) && $elementLessThan!="")
					$jsOutput[] = "checkUBInt('$elementName', {$elementLessThan}, '$elementHelpName')";
			}
			if(is_numeric($elementSize) && $elementSize > 0) {
