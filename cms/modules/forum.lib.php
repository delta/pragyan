 ...
 *
 * To be Done : 1.create threads(code for the sticky threads also) and post replies CODE
...
class forum implements module {
	...
	public function actionModerate() {
		global $urlRequestRoot, $moduleFolder, $sourceFolder,$templateFolder;
		...
		$templatesImageFolder = "$urlRequestRoot/$sourceFolder/$templateFolder/".TEMPLATE;
		...
		if (!isset ($_GET['forum_id'])) {
			$query = "SELECT * FROM `$table_name` WHERE `page_modulecomponentid`='" . $this->moduleComponentId . "" .
					"' AND `forum_thread_category`='general' ORDER BY `forum_thread_datetime` DESC";
			$result = mysql_query($query);
			...
			if ($result) {
				...
				$moderate =<<<PRE
		<p align="left"><a href="$action" style="color:#0F5B96"><img src="$temp/new_post.gif" /></a></p>
...
PRE;
			...
			$subaction = strtolower($text);
			$postpart =<<<PRE
        <p align="left"><a href="+post&subaction=post_reply&thread_id=$forum_id"><img src="$temp/post_reply.gif" /></a>&nbsp;
        <a href="+post&subaction=create_thread"><img src="$temp/new_post.gif" /></a></p>
		...
		<img src="$temp/$img" /></a></td>
		<td class="forumTableRow"><strong>$forum_topic</strong><br /><img src="$temp/icon_post_target.gif" />
...
PRE;
			if ($userId > 0 && $name != "Anonymous") {
				if ($rows['user_id'] == $userId)
					...
					if($moderator)$postpart .= "Modearator";else
				$postpart .= "Member";
				$postpart .= '</strong><br /><br /><small>Posts: ' . $posts . ' <br />Joined: ' . $reg_date . ' <br />Last Login:' . $lastLogin .
'</small>';
			}
			$postpart .=<<<PRE
	        </td>
...
PRE;
	        ...
	        <td class="forumTableRow"><strong>Re:- $post_title </strong><br /><img src="$temp/icon_post_target.gif" />
...
PRE;
			...
			$postpart .= '<br>
			        <p align="left"><a href="+post&subaction=post_reply&thread_id='.$forum_id.'"><img src="'.$temp.'/post_reply.gif" />' .
			        		'</a>&nbsp;<a href="+post&subaction=create_thread"><img src="'.$temp.'/new_post.gif" /></a></p>';
			return $postpart;
		}
	}
				...
				if ($subaction == "post_reply") {
					if (!$_POST['subject'] || !$_POST['message']) {
						...
						if ($result) {
							...
							if($rows1['forum_access_status']=="moderated")
	...
	public function actionView() {
		...
		if (!isset ($_GET['thread_id'])) {
			...
			$query = "SELECT * FROM `$table_name` WHERE `page_modulecomponentid`='" . $this->moduleComponentId . "' AND " .
					"`forum_thread_category`='general' ORDER BY `forum_thread_datetime` DESC";
			$result = mysql_query($query);
	        ...
	        <table width="100%" border="0" align="center" cellpadding="3" cellspacing="1">
	        <tr>
	        <td class="forumTableHeader" colspan="2"><strong>Subject</strong><br /></td>
	        <td class="forumTableHeader"> <strong>Views</strong></td>
	        <td class="forumTableHeader"><strong>Replies</strong></td>
	        <td class="forumTableHeader"><strong>Last Post</strong></td>
	        </tr>
...
PRE1;
			...
			$reg_date = $this->getRegDateFromUserID($rows['forum_thread_user_id']);
			$postpart =<<<PRE
        <p align="left"><a href="+post&subaction=post_reply&thread_id=$thread_id"><img src="$temp/post_reply.gif" /></a>&nbsp;
...
PRE;
			if ($rows['forum_post_approve'] == 1) {
				$postpart .= '<tr>
					        <td class="forumTableRow"><strong> ' . $forum_topic . ' </strong><br />' .
					        		'<img src="' . $temp . '/icon_post_target.gif" /><small style="font-size:9px;">by ' . $name . ' </a>' .
					        				' on ' . $rows[forum_thread_datetime] . ' </small></td>
				...
				if ($threadUserId > 0) {
					...
					if($moderator)$postpart .= "Moderator";else
					$postpart .= "Member";
					$postpart .= '</strong><br /><br /><small>Posts: ' . $posts . ' <br />Joined: ' . $reg_date . ' <br />Last Login:'
					. $lastLogin . '</small>';
				}
				$postpart .=<<<PRE
	        </td>
	        ...
	        <tr>
	        <td bgcolor="#F8F7F1" style="font-size:11px;"> <br />$forum_detail </td>
	        </tr>
...
PRE;
			...
			$postpart .= '<br>
			        <p align="left"><a href="+post&subaction=post_reply&thread_id='.$thread_id.'">' .
			        		'<img src="'.$temp.'/post_reply.gif" /></a>&nbsp;<a href="+post&subaction=create_thread">' .
			        				'<img src="'.$temp.'/new_post.gif" /></a></p>';
			return $postpart;
		}
	}
...
PRE;
			if ($rows['forum_post_approve'] == 1) {
				$postpart .= '<tr>
					        <td bgcolor="#F8F7F1" style="font-size:13px;"><strong> ' . $forum_topic . ' </strong><br />' .
					        		'<img src="' . $temp . '/icon_post_target.gif" /><small style="font-size:9px;">by ' . $name . ' </a> ' .
					        				'on ' . $rows[forum_thread_datetime] . ' </small></td>
					        <td bgcolor="#F8F7F1"  width="20%" rowspan="2" valign="top" width="20%"><strong> ' . $name . ' </a><br />';
				if ($threadUserId > 0) {
					if ($threadUserId == $userId)
						$lastLogin = $_SESSION['last_to_last_login_datetime'];
					else
...
PRE;
				if ($rows['forum_post_approve'] == 1) {
					$postpart .= '<tr>
						        <td bgcolor="#F8F7F1" style="font-size:13px;"><strong>Re:-' . $post_title . '</strong><br />' .
						        		'<img src="' . $temp . '/icon_post_target.gif" /><small style="font-size:9px;">by ' . $name . ' </a>' .
						        				' on ' . $rows['forum_post_datetime'] . ' </small></td>
						        <td bgcolor="#F8F7F1"  width="20%" rowspan="2" valign="top" width="20%"><strong>' . $name . '</a><br />';
					if ($postUserId > 0) {
	        ...
	        <tr>
	        <td bgcolor="#F8F7F1" style="font-size:11px;"><br />$post_content</td>
	        </tr>
PRE;
					if ($userId > 0 && $userId == $rows['forum_post_user_id'])
					 //compare the userID of the logged in user with that of the author of the current reply
						{
						$postpart .= '<tr><td bgcolor="#F8F7F1" colspan="2" align="right">' .
								'<a href="+view&subaction=delete_post&thread_id=' . $thread_id . '&post_id=' . $rows['forum_post_id'] . '">' .
										'<img src="'.$temp.'/delete_sm.gif"</a></span></td></tr>';
					}
				}
			...
			$postpart .= '<br>
			        <p align="left"><a href="+post&subaction=post_reply&thread_id='.$thread_id.'"><img src="'.$temp.'/post_reply.gif" />' .
			        		'</a>&nbsp;<a href="+post&subaction=create_thread"><img src="'.$temp.'/new_post.gif" /></a></p>';
			return $postpart;
	}
