...
class article implements module, fileuploadable {
	...
	public function actionEdit() {
		...
		submitFileUploadForm($this->moduleComponentId,"article",$this->userId,15*1024*1024);
		if (isset($_GET['preview']) && isset ($_POST['FCKeditor1'])) {
			return "<div id=\"preview\" class=\"warning\"><a name=\"preview\">Preview</a></div>".$this->actionView(stripslashes($_POST[FCKeditor1])).$this->getFckBody(stripslashes($_POST[FCKeditor1]));
		}
		if (isset($_GET['version'])) {
			$revision = $this->getRevision($_GET['version']);
			return "<div id=\"preview\" class=\"warning\"><a name=\"preview\">Previewing Revision Number ".$_GET['version']."</a></div>".$this->actionView($revision).$this->getFckBody($revision);
		}
		...
		/*Save the diff :-*/
			...
			$diff = mysql_escape_string($this->diff($_POST['FCKeditor1'],$row['article_content']));
...
VALUES ('$this->moduleComponentId', '$revId','$diff','$this->userId')";
		...
		/*Save the diff end.*/
			...
			$query = "UPDATE `article_content` SET `article_content` = '" . $_POST[FCKeditor1] . "' WHERE `page_modulecomponentid` =$this->moduleComponentId ";
			$result = mysql_query($query);
			if(mysql_affected_rows() < 1)
				displayerror("Unable to update the article");
			return $this->actionView();
		}
		return $this->getFckBody();
...
//		return $originalArticle;
	...
	public function getFckBody($content=""){
			global $sourceFolder;
			global $moduleFolder;
			global $urlRequestRoot;
			require_once ("$sourceFolder/$moduleFolder/article/fckEditor/fckeditor.php");
			if($content=="") {
...
FCK;
			...
			$oFCKeditor->BasePath = "$urlRequestRoot/$sourceFolder/$moduleFolder/article/fckEditor/";
			$oFCKeditor->Value = $content;
			$oFCKeditor->Width = '100%';
			$oFCKeditor->Height = '600';
			$fckbody = $oFCKeditor->Create();
			...
			$fckFooter =<<<FCK1
					    ...
					    <fieldset>
							...
							Uploaded Files : <br />
FCK1;
		$fckFooter .= getUploadedFilePreviewDeleteForm($this->moduleComponentId,"article",'./+edit');
		$fckFooter .= '<br />Upload files : <br />'.getFileUploadForm($this->moduleComponentId,"article",'./+edit',15*1024*1024,5).'</fieldset>';
		...
		return  $fckForm . $fckbody . $fckFooter.$revisionTable;
	}
