...
class pr implements module {
	...
	private function getNewUserThirdPartyRegistrationForm() {
		global $sourceFolder, $moduleFolder, $urlRequestRoot;

		$registrationForm = <<<REGISTRATIONFORM
			<script language="javascript" type="text/javascript" src="$urlRequestRoot/$sourceFolder/$moduleFolder/pr/pr.js">
			</script>
...
REGISTRATIONFORM;
	...
	private function submitNewUserThirdPartyRegistrationForm() {
		if(
			...
			$userIdQuery = 'SELECT MAX(`user_id`) FROM `pragyanV2_users`';
			...
			$insertQuery = 'INSERT INTO `pragyanV2_users`(`user_id`, `user_name`, `user_email`, `user_fullname`, `user_password`, `user_regdate`, `user_lastlogin`, `user_activated`) ' .
