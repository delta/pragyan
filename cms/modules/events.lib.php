...

{
	...
	function getHtml($gotuid, $gotmoduleComponentId, $gotaction)
	{
		$this->userId = $gotuid;
		$this->moduleComponentId = $gotmoduleComponentId;
		if ($gotaction == 'csg') return $this->actionCsg();
		if ($gotaction == 'eventshead') return $this->actionEventshead();
		if ($gotaction == 'ochead') return $this->actionOchead();
		if ($gotaction == 'octeam') return $this->actionOcteam();
		if ($gotaction == 'qa') return $this->actionQa();
		if ($gotaction == 'qahead') return $this->actionQahead();
		if ($gotaction == 'pr') return $this->actionPr();
		if ($gotaction == 'prhead') return $this->actionPrhead();
		if ($gotaction == 'view') return $this->actionView();
		/*    if ($gotaction == '')
		...
		if (isset($_GET['subaction']))
		{
			...
			if ($_GET['subaction'] == 'getParticipant')
			{
				if (isset($_POST['eventId']))
				{
					...
					{
						if ($userId > 200000 && $userId < 30000) $userId-= 180000;
...
FORM;
		...
		else return displayQa($moduleComponentId);
	}

	public

	function actionQahead()
	{
		global $urlRequestRoot, $sourceFolder, $templateFolder, $cmsFolder, $moduleFolder;
		$moduleComponentId = $this->moduleComponentId;
		$userId = $this->userId;
...
FORM;
	...
	function actionPrhead()
	{
		...
		else return prMain('prhead', $moduleComponentId);
	}
	...
	function deleteModule($moduleComponentId)
	{
		return true;
	}
	...
	function copyModule($moduleComponentId, $newId)
	{
		return true;
	}
