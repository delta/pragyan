	...
	public function actionEdit($moduleComponentId) {
		...
		if (is_array($uploadSuccess) && isset ($uploadSuccess[0])) {
			for($i=0;$i<count($uploadSuccess);$i++){
				$gallQuery3 = "INSERT INTO `gallery_pics` (`upload_filename`, `page_modulecomponentid`, `gallery_filecomment`) VALUES('$uploadSuccess[$i]', $this->moduleComponentId, '{$_POST['desc']}')";
		...
		$content2 .=<<<GALFORM
					...
					<table>
						<tr><td>Edit gallery name and description</td></tr>
						<tr>
							<td>Gallery Name</td>
		...
		if ($fileArray) {
			for ($i = 0; $i < count($fileArray); $i++) {
				...
				<span style="float:left">
					...
					<br/><input type="submit" name="btnEditComment" value="Edit comment">
					<br/><input type="submit" name="btnDeleteImage" value="Delete" />
				</span>
IMGFORM;
				$content2 .= "</form>";
			}
