	...
	if(isset($_POST['btnSubmitProfile'])) {
		if($forEditRegistrant || !isProfileFormCaptchaEnabled() || submitCaptcha()) {
			...
			if (isset($_POST['user_fullname']) && $_POST['user_fullname'] != '' && $_POST['user_fullname'] != $userFullname) {
				...
				$newUserFullname = $_POST['user_name'];
			}
			$errors = true;
...
function getProfileForm($userId, $userName, $userFullname, $forEditRegistrant = false) {
	...
	$fValidatorPath = "$urlRequestRoot/$sourceFolder/$templateFolder/common/scripts/formValidator.js";
	$abhiValidatorPath = "$urlRequestRoot/$sourceFolder/$moduleFolder/form/validation.js";
...
<div class="registrationform">
	<form id="registrationform" class="fValidator-form" method="POST" name="user_profile_usrFrm" onsubmit="return checkProfileForm(this)" action="$formAction">
...
function getProfileFormEditForm() {
	...
	return generateFormDescBody($moduleComponentId, 'admin&subsubaction=editprofileform') . '<br /><br />' .
			generateFormElementDescBody(0, 'admin&subsubaction=editprofileform');
}
...
function getProfileRegistrantsList($showEditButtons = false) {
	...
	$normalImage = "<img alt=\"Sort by this field\" height=\"12\" width=\"12\" style=\"padding:0px\" src=\"$urlRequestRoot/$sourceFolder/$templateFolder/common/icons/16x16/actions/view-refresh.png\" />";
	$orderedImage = "<img alt=\"Sort by this field\" height=\"12\" width=\"12\" style=\"padding:0px\" src=\"$urlRequestRoot/$sourceFolder/$templateFolder/common/icons/16x16/actions/go-" . ($sortOrder == 'asc' ? 'up' : 'down') . ".png\" />";
...
function getProfileGroupsAndFormsList($userId) {
	...
	if(count($unassociatedGroups) > 0) {
		$retVal .= '<strong>Groups not associated with any form:</strong><br /><table style="margin-left: 8px" border="1" cellpadding="4px" cellspacing="4px">' .
						'<tr><th>Group Name</th><th>Group Description</th></tr>' .
