...
function showPagePath(e) {
	if(this.className != 'ddtree_inaccessible' && this.className != 'ddtree_selected') {
		if(inputBoxId != '')
			document.getElementById(inputBoxId).value = this.title;
		this.className = 'ddtree_selected';
		if(prevSelected) {
			prevSelected.className = 'ddtree_accessible';
		}
		prevSelected = this;
	}
...
function setupMenuDependencies(imgDir, inputId) {
	ddtreemenu.closefolder = imgDir + '/ddtree_closed.gif';
	ddtreemenu.openfolder= imgDir + '/ddtree_open.gif';
	inputBoxId = inputId;
}

function treeLinkClicked(e) {
	if(typeof e != "undefined") {
		e.stopPropagation();
	}
	else {
